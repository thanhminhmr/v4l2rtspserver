...
bool HTTPServer::HTTPClientConnection::sendMpdPlayList(char const *urlSuffix) {
	...
	os << "<?xml version='1.0' encoding='UTF-8'?>\r\n"
	   << "<MPD type='dynamic' xmlns='urn:mpeg:DASH:schema:MPD:2011' profiles='urn:mpeg:dash:profile:full:2011' "
		  "minimumUpdatePeriod='PT"
	   << sliceDuration << "S' minBufferTime='" << sliceDuration << "'>\r\n"
	   << "<Period start='PT0S'><AdaptationSet segmentAlignment='true'><Representation mimeType='video/mp2t' codecs='' "
		  ">\r\n";

	os << "<SegmentTemplate duration='" << sliceDuration << "' media='" << urlSuffix
	   << "?segment=$Number$' startNumber='" << startTime << "' />\r\n";
...
std::list<std::string> getSubsessionFormats(ServerMediaSession *session) {
...
void HTTPServer::HTTPClientConnection::handleHTTPCmd_StreamingGET(char const *urlSuffix, char const *fullRequestStr) {
	...
	if (strcmp(urlSuffix, "version") == 0) {
		...
		ServerMediaSession *serverSession = nullptr;
		while ((serverSession = it.next()) != nullptr) {
			if ((streamName == serverSession->streamName()) || streamName.empty()) {
				break;
			}
		}
		BaseServerMediaSubsession *baseSubsession = nullptr;
		...
		this->sendHeader("text/plain", content.size());
		this->streamSource(content);
	} else if (questionMarkPos == nullptr) {
		std::string streamName(urlSuffix);
		std::string ext;
		...
		// Call "getStreamParameters()" to create the stream's source.  (Because we're not actually streaming via
		// RTP/RTCP, most of the parameters to the call are dummy.)
		++m_ClientSessionId;
		...
		subsession->getStreamParameters(
				m_ClientSessionId, clientAddress, clientRTPPort, clientRTCPPort, -1, 0, 0, destinationAddress,
				destinationTTL, isMulticast, serverRTPPort, serverRTCPPort, m_StreamToken
		);
#else

		subsession->getStreamParameters(
				m_ClientSessionId, clientAddress, clientRTPPort, clientRTCPPort, -1, 0, 0, nullptr, destinationAddress,
				destinationTTL, isMulticast, serverRTPPort, serverRTCPPort, m_StreamToken
		);
#endif
...
void HTTPServer::HTTPClientSession::handleCmd_SETUP(
