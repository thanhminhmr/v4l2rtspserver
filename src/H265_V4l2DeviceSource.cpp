...
H265_V4L2DeviceSource::splitFrames(std::uint8_t *frame, unsigned frameSize) {
	...
	while (buffer != nullptr) {
		...
		case 32:
			...
			m_vps.assign(reinterpret_cast<const char *>(buffer), size);
			m_sps.clear();
			m_pps.clear();
			break;
		case 33:
		...
		if (!m_vps.empty() && !m_sps.empty() && !m_pps.empty()) {
			std::unique_ptr<char[]> vps_base64(base64Encode(m_vps.c_str(), m_vps.size()));
			...
			os << "sprop-vps=" << vps_base64.get();
			os << ";sprop-sps=" << sps_base64.get();
